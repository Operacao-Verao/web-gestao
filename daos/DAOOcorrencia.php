<?php
    class DAOOcorrencia {
        ...
        public function insert(Tecnico|null $tecnico, Civil $civil, Casa $casa, string $acionamento, string $relatoCivil, int $numCasas, bool $aprovado, bool $encerrado, string $dataOcorrencia): ?Ocorrencia{
            // Try to insert the provided data into the database
            $insertion = $this->pdo->prepare("INSERT INTO Ocorrencia (id_tecnico, id_civil, id_casa, acionamento, relato_civil, num_casas, aprovado, encerrado, data_ocorrencia) VALUES (:idTecnico, :idCivil, :casa, :acionamento, :relatoCivil, :numCasas, :aprovado, :encerrado, :dataOcorrencia)");
            ...
            if ($insertion->execute()) {
                ...
                return new Ocorrencia($last_id, $tecnico? $tecnico->getId(): null, $civil->getId(), $casa->getId(), $acionamento, $relatoCivil, $numCasas, $aprovado, $encerrado, $dataOcorrencia);
            }
        ...
        public function findById(int $id): ?Ocorrencia{
            ...
            if ($queries) {
                $query = $queries[0];
                return new Ocorrencia($id, $query['id_tecnico'], $query['id_civil'], $query['id_casa'], $query['acionamento'], $query['relato_civil'], $query['num_casas'], $query['aprovado'], $query['encerrado'], $query['data_ocorrencia']);
            }
            return null;
        }
        ...
        public function listAll(): ?array{
            ...
            if ($queries) {
                ...
                foreach ($queries as $query) {
                    $models[] = new Ocorrencia($query['id'], $query['id_tecnico'], $query['id_civil'], $query['id_casa'], $query['acionamento'], $query['relato_civil'], $query['num_casas'], $query['aprovado'], $query['encerrado'], $query['data_ocorrencia']);
                }
                return $models;
            }
            return [];
        ...
        public function searchByText(string $text, bool $status): ?array{
            ...
            $statement = $this->pdo->query('select * from Casa, Endereco, Ocorrencia where (Ocorrencia.id_casa=Casa.id and Casa.cep=Endereco.cep) and (Ocorrencia.relato_civil like "%'.$text.'%" or Endereco.rua like "%'.$text.'%" or Endereco.bairro like "%'.$text.'%" or Casa.numero like "'.$text.'%") and aprovado = '.($status?1:0).' order by Ocorrencia.data_ocorrencia desc');
            $queries = $statement->fetchAll(PDO::FETCH_ASSOC);
        ...
        public function update(Ocorrencia $ocorrencia): bool{
            $update = $this->pdo->prepare("UPDATE Ocorrencia SET id_tecnico = :idTecnico, id_civil = :idCivil, id_casa = :id_casa, acionamento = :acionamento, relato_civil = :relatoCivil, num_casas = :numCasas, aprovado = :aprovado, encerrado = :encerrado, data_ocorrencia = :dataOcorrencia WHERE id = :id");
            ...
            $update->bindValue(":id_casa", $ocorrencia->getIdCasa());
            ...
            $update->bindValue(":encerrado", $ocorrencia->getAprovado());
