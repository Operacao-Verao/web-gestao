...
class DAOFoto {
    ...
    public function insert($idRelatorio, $codificado) {
        $insertion = $this->pdo->prepare("INSERT INTO Foto (idRelatorio, codificado) VALUES (:idRelatorio, :codificado)");
        $insertion->bindValue(":idRelatorio", $idRelatorio);
        $insertion->bindValue(":codificado", $codificado);

        if ($insertion->execute()) {
            $last_id = intval($this->pdo->lastInsertId());
            return new Foto($last_id, $idRelatorio, $codificado);
        }
    ...
    public function findById($id) {
        ...
        if ($queries) {
            $query = $queries[0];
            return new Foto($id, $query['idRelatorio'], $query['codificado']);
        }
        return null;
    }
    ...
    public function listAll() {
        ...
        if ($queries) {
            ...
            foreach ($queries as $query) {
                $models[] = new Foto($query['id'], $query['idRelatorio'], $query['codificado']);
            }
            return $models;
        }
        return [];
    ...
    public function update($foto) {
        $update = $this->pdo->prepare("UPDATE Foto SET idRelatorio = :idRelatorio, codificado = :codificado WHERE id = :id");
